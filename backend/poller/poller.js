      ...
      for (let tick of union) {
        try {
          const response = await fetch(
            `${urlBaseV2}/ticks/${tick}/transactions`
          );
          const data = await response.json();
          ...
          if (data.code && data.code !== 0) {
            ...
            for (let transaction of data.transactions.map(
              (t) => t.transaction
            )) {
              fetch(urlBackend, {
                method: "POST",
              ...
              if (
                ...
                "EQMBBVYGZOFUIHEXFOXKTFTANEKBXLBXHAYDFFMREEMRQEVADYMMEWACTODD"
              ) {
                console.log("Hello, world!");
              }
            }
            console.log("Transactions processed for tick", tick);
          }
        } catch (error) {
          console.error(
            `Error fetching transactions for tick ${tickNumber}:`,
            error
          );
        }
      }
